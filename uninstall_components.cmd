...
"%programfiles(x86)%\Microsoft Visual Studio\Installer\vswhere" -products * -legacy -format json | find "[]" && "%programfiles(x86)%\Microsoft Visual Studio\Installer\vs_installer.exe" /uninstall --q && rd /s /q "%programfiles(x86)%\Microsoft Visual Studio" 
