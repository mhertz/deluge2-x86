...
for /f %%i in ('"%programfiles(x86)%\Microsoft Visual Studio\Installer\vswhere.exe" -version [16.0^,17.0^) -prerelease -products * -property installationPath') do call "%%i\VC\Auxiliary\Build\vcvars32.bat"
pushd boost
call bootstrap.bat
popd
...
b2 crypto=openssl libtorrent-link=static boost-link=static release optimization=speed stage_module --abbreviate-paths -j4 address-model=32 openssl-include=%OPENSSL%\include openssl-lib=%OPENSSL%\lib cflags="/GL" linkflags="/LTCG"
popd
